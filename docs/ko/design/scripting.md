...
스크립트 훅이 실행되면 배포 흐름은 위자드 언어를 `SHUN_LANGUAGE`
환경 변수로 모든 스크립트 단계에 내보냅니다(위자드 첫 단계에서
선택한 로캘이며, 디스크의 설치 매니페스트에도 기록됩니다). 언어를
선택하지 않았다면 변수 자체가 없습니다. 사실을 내보내는 것이 shun이
하는 전부이고, 설치된 애플리케이션 자체 설정에 언어를 쓰는 것은
payload 스크립트의 몫입니다.
