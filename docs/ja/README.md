...
現在のリリース：**0.2.1**。crate は celestia エコシステムの 3 つの実消費者——WoWSP インストーラーシェル、shittim-chest ローカル、evernight イメージフラッシャー——に合わせて安定化中。API はマイナーバージョン間でこの 3 消費者に追従——統合フィードバックによる追加変更を想定。
