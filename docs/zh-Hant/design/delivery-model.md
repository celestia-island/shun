...
- **install** —— 各平台直接註冊：Windows 為使用者級 ARP 條目、自複製解除安裝器、開始功能表/桌面捷徑（帶 AUMID）與可選的 Explorer 右鍵動詞；Linux 為使用者級 `.desktop` 啟動器 + 桌面動作（含解除安裝）；macOS 為 `.app` 包補全 + Launch Services 註冊。便携模式在任何平台都不碰系統狀態。解除安裝按清單清除全部痕跡。
